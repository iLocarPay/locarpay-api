...
// SEC-CONTRACT-03B1: handleAsaasPayment removido. Este webhook não confirma pagamentos Asaas —
// isso é feito exclusivamente pelos webhooks Asaas dedicados (/payment-webhook, /billing-webhook),
// que validam o header asaas-access-token contra seus próprios segredos.
...
export default async function handler(req, res) {
  ...
  // SEC-CONTRACT-03B1: a Assinafy NÃO é autoridade de pagamento Asaas. Este webhook processa
  // exclusivamente eventos de assinatura de documento. Qualquer evento de pagamento (PAYMENT_*,
  // que jamais é enviado pela Assinafy — a conta registra apenas document_ready) é rejeitado
  // sem escrita financeira. As baixas Asaas ocorrem só nos webhooks Asaas dedicados
  // (/payment-webhook e /billing-webhook, autenticados por header asaas-access-token).
  const upperEvent = (payload.event || payload.type || '').toUpperCase();
  if (payload.payment?.id || upperEvent.startsWith('PAYMENT_') || upperEvent.startsWith('SUBSCRIPTION_')) {
    console.warn('[assinafy-webhook] evento de pagamento rejeitado neste endpoint (fail-closed):', upperEvent);
    return res.status(200).json({ ok: true, skipped: 'evento de pagamento não é processado pelo webhook de assinatura' });
  }
  ...
  // Allowlist explícita: só eventos de assinatura conhecidos prosseguem.
  const SIGNATURE_EVENTS = ['document_ready', 'document_completed', 'document.completed', 'assignment.completed', 'completed', 'finished', 'signer_signed_document'];
  if (!SIGNATURE_EVENTS.some(e => event.includes(e))) {
    console.log(`[assinafy-webhook] evento '${event}' fora da allowlist de assinatura — ignorado`);
    return res.status(200).json({ ok: true, skipped: `evento '${event}' não é de assinatura` });
  }
