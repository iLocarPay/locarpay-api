...
// SEC-FIN-02B2: identidade do INQUILINO a partir do token (nunca do corpo).
// 401 sem/inválido/revogado; 403 se a conta estiver suspensa (users/{uid}.suspended === true).
export async function requireTenant(db, req) {
  const auth = await verifyBearer(req);
  await assertActiveUser(db, auth);
  return auth; // { uid, email }
}

// Ownership da cobrança pelo inquilino autenticado — mesmo critério das Firestore Rules:
// charge.tenantId === uid OU charge.tenantEmail === email do token.
export function chargeOwnedByTenant(charge, auth) {
  if (!charge || !auth) return false;
  const tId = typeof charge.tenantId === 'string' ? charge.tenantId : '';
  const tEmail = (typeof charge.tenantEmail === 'string' ? charge.tenantEmail : '').toLowerCase();
  return (!!tId && tId === auth.uid) || (!!tEmail && tEmail === auth.email);
}

// SEC-FIN-02B3: autoriza uma identidade autenticada sobre uma cobrança REAL, resolvendo
// pelo token (nunca pelo corpo) se o chamador é master, o inquilino dono da cobrança ou o
// owner (imobiliária) dono da cobrança. Retorna 'master' | 'tenant' | 'owner'; senão 403.
export async function authorizeChargeAccess(db, auth, charge) {
  if (isMasterEmail(auth.email)) return 'master';
  if (chargeOwnedByTenant(charge, auth)) return 'tenant';
  const ownerId = charge && typeof charge.ownerId === 'string' ? charge.ownerId : '';
  if (ownerId) {
    try { await assertOwner(db, auth, ownerId); return 'owner'; } catch (_) { /* fail-closed abaixo */ }
  }
  throw Object.assign(new Error('Acesso negado'), { status: 403 });
}

// SEC-CONTRACT-03B2: master OU owner do ownerId OU corretor legítimo desse owner.
// Corretor = brokers/{}.email == e-mail do token E ownerId == ownerId do recurso; se o recurso
// registra brokerEmail (corretor responsável), precisa ser exatamente ele. Nada do corpo decide.
// Retorna 'master' | 'owner' | 'broker'; senão 403 (fail-closed).
export async function assertOwnerOrBroker(db, auth, ownerId, brokerEmail) {
  if (isMasterEmail(auth.email)) return 'master';
  if (!ownerId || typeof ownerId !== 'string') throw Object.assign(new Error('Acesso negado'), { status: 403 });
  try { await assertOwner(db, auth, ownerId); return 'owner'; } catch (_) { /* tenta corretor abaixo */ }
  const responsible = (typeof brokerEmail === 'string' ? brokerEmail : '').toLowerCase();
  if (responsible && responsible !== auth.email) throw Object.assign(new Error('Acesso negado'), { status: 403 });
  const q = await db.collection('brokers').where('email', '==', auth.email).where('ownerId', '==', ownerId).limit(1).get();
  if (q.empty) throw Object.assign(new Error('Acesso negado'), { status: 403 });
  const b = q.docs[0].data() || {};
  if (b.active === false || b.suspended === true) throw Object.assign(new Error('Acesso negado'), { status: 403 });
  return 'broker';
}
