// PROPERTIES-02A — Módulo Imóveis: serviço server-side (autorização + isolamento).
//
// MÓDULO, não endpoint: não exporta handler HTTP e não cria Serverless Function (plano Hobby
// permite 12; ver PROPERTIES-02A-FN-LIMIT). É exposto por api/ilocarpay-broker.js nos steps
// property-create | property-get | property-list | property-update | property-archive |
// property-restore, todos POST com Bearer (Firebase ID token) OBRIGATÓRIO.
//
...
import { verifyBearer, isMasterEmail, assertActiveUser, assertOwner } from './authz.js';
// Mesmo limitador usado pelos demais endpoints (api/_security.js é módulo compartilhado,
// não vira função por causa do prefixo "_").
...
export async function handleCreate(db, ctx, body) {
...
export async function handleGet(db, ctx, body) {
...
export async function handleList(db, ctx, body) {
...
export async function handleUpdate(db, ctx, body) {
...
export async function handleArchive(db, ctx, body) {
...
export async function handleRestore(db, ctx, body) {
...
// Steps públicos aceitos por api/ilocarpay-broker.js (nomes fechados, sem colisão com os existentes).
export const PROPERTY_STEPS = new Set(Object.keys(STEPS).map((s) => 'property-' + s));

// Adaptador: único ponto que enxerga req. Autentica UMA vez (verifyBearer dentro de
// resolveCaller), aplica o rate limit de escrita do módulo e delega à operação do step.
// Erros carregam { status } e mensagem fixa; quem chama devolve ao cliente.
export async function handlePropertyStep(db, req, step) {
  if (!PROPERTY_STEPS.has(step)) throw err('step inválido', 400);
  const op = step.slice('property-'.length);
  const write = WRITE_STEPS.has(op);
  try {
    const ctx = await resolveCaller(db, req, { write });
    if (write) await rateLimit(`properties:${ctx.auth.uid}`, WRITE_RATE);
    return await STEPS[op](db, ctx, req.body || {});
  } catch (e) {
    // Erros esperados já têm { status } e mensagem fixa do módulo (sem PII) — sobem como estão.
    // Falha inesperada NÃO pode vazar detalhe interno pela resposta de quem chama (o catch do
    // broker devolve e.message): o detalhe fica no log e o cliente recebe 500 genérico.
    if (e && e.status) throw e;
    console.error('[properties]', e && e.message);
    throw err('Erro interno', 500);
  }
}
