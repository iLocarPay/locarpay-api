...
/**
 * SEC-FIN-02B1: resolve owner e chave Asaas EXCLUSIVAMENTE a partir da cobrança.
 * Fail-closed: sem ownerId na charge, owner inexistente ou sem asaasApiKey própria → erro.
 * NUNCA cai para owner default nem para a chave master (config/asaas).
 * O cliente não pode escolher a credencial financeira.
 */
export async function resolveChargeOwnerKey(db, charge) {
  const ownerId = charge && typeof charge.ownerId === 'string' ? charge.ownerId.trim() : '';
  if (!ownerId) throw Object.assign(new Error('Owner da cobrança não resolvido'), { status: 409 });
  const snap = await db.collection('owners').doc(ownerId).get();
  if (!snap.exists) throw Object.assign(new Error('Owner da cobrança não encontrado'), { status: 409 });
  const owner = snap.data() || {};
  const apiKey = owner.asaasApiKey;
  if (!apiKey || typeof apiKey !== 'string') throw Object.assign(new Error('Chave Asaas do owner não configurada'), { status: 409 });
  return { ownerId, apiKey, owner };
}
